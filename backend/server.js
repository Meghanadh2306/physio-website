...
app.get("/patients", auth, async (req, res) => {
  ...
  if (doctor) andConditions.push({ recommendedDoctor: doctor });
...
app.get("/doctor/report/excel", auth, async (req, res) => {
  try {
    ...
    const headerRow = ws.addRow(['PHONE', 'PATIENT NAME', 'DAYS', 'PAYMENT', 'REF FEE']);
    ...
    console.error("DOCTOR EXCEL ERROR:", err);
    res.status(500).send("Error generating Doctor Excel report");
  }
});
...
app.get("/doctor/report/pdf", async (req, res) => {
  try {
    ...
    pts.forEach(p => {
      ...
      if (continueText) doc.text(continueText, 42, y + 12, { width: 151, align: 'center' });
    ...
    if (y === startY + 25) {
      doc.font("Times-Bold").fontSize(12).text("NO PATIENTS FOUND FOR THIS PERIOD.", 40, y + 10, { align: 'center', width: 515 });
    }
    ...
    doc.text(`Rs. ${grandRefFee.toFixed(2)}`, 430, y, { align: 'center', width: 120 });
